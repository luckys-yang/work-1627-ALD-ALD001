...
{
	UINT8 fd = DOS_FileOpen("D:\\1628.BIN", 'r');
	...
	if (!fd) {
		...
		switch (DIR_Change("D:\\DCIM\\100MEDIA\\")) {
			case DOS_SUCCESS:
				//printf("folder change : success\n");
				break;

			case DOS_ERR_INVALID_PATH:
				//printf("folder change : invalid path\n");
				break;

			case DOS_FILE_NOT_FOUND:
				//printf("folder change : file not found\n");
				break;
		}
	...
	if(sp1kCardFirmwareUpdate("D:\\1628.BIN" , 0)==FAIL)
	{
		app_set_led(0);
		sp1kHalWait(200);
		app_set_led(1);
	...
	if (averageAfValue > 10) {
		...
		if (stableCnt == 1) {
			...
			if (MaxIdx && MaxIdx  <= difference) {
				...
				idx = 0;
				timeoutCnt = 20;
				memset(afInfos, 0x00, sizeof(afInfos));
...
void AF_StopCalAf(void) {
	if (AFCalAfFlag) {
		...
		//printf("AF Cal Af Stop\n");
	}
}

#if (defined SUPPORT_AUTO_AF) && SUPPORT_AUTO_AF

#if (defined AUTO_AF_CHECK_MD) && AUTO_AF_CHECK_MD
static UINT8 AF_MD = 0;

extern UINT8 AF_MdAction1SecCnt;
extern UINT8 AF_MdAction1SecCntStartFlag;
extern UINT8 AF_MdAction1SecDoneFlag;

void AF_MdAfEn(UINT8 en) {
	AF_MD = en ? 1 : 0;

	#if 0//SUPPORT_SENSOR_SWITCH
	AF_MD = sensor_switch ? AF_MD : 0;
	#endif

	if (!AF_MD) {
		AF_StopCalAf();

		AF_MdAction1SecCnt = 0;
		AF_MdAction1SecCntStartFlag = 0;
		AF_MdAction1SecDoneFlag = 0;
	}
}

void AF_SnapBeforeMdAfStop(void) {
	if (AF_MD) {
		AF_MD = 0;
...
UINT8 AF_MdAfEnGet(void) {
	return AF_MD;
}
#endif


#endif 		// #if (defined SUPPORT_AUTO_AF) && SUPPORT_AUTO_AF
