...
void appEVSet(UINT8 expval)
{
	...
	switch(expval)
	{
		...
		case PRV_COMP_0EV:
			...
			// sp1kAeParamSet(SP1K_AE_EV_COMP, SP1K_AE_EXP_COMP_M02);
			break;
		case PRV_COMP_P03EV:
		...
		case PRV_COMP_P17EV:
			...
			break;
		case PRV_COMP_P20EV:
			sp1kAeParamSet(SP1K_AE_EV_COMP, SP1K_AE_EXP_COMP_P20);
			break;
	}
}
...
void uiUpdateRemainPix(void)
{
	...
	sp1kOsdStrDrawExt(LCD_WIDTH - chrw*5-1, y, "            ", 0);	
	sp1kOsdStrDrawExt(LCD_WIDTH -strWidthGet(str)-1 -14, y, str, 0);
...
{ 
	...
	sp1kOsdStrDrawExt(LCD_WIDTH - chrw*4-1, y, "            ", 0);	
	sp1kOsdStrDrawExt(LCD_WIDTH -strWidthGet(str)-1, y, str, 0);
}
...
void uiOsdDialogDisp(UINT8 stridx, UINT16 wait)
{
	...
	sp1kOsdClearCharExt(55, 57, 220, 122);

	//if(sp1kSD_CdGet()&&(appCurrStateGet() == APP_STATE_STILL_VIEW)){
	//}
...
void appCurrentTimeShow(UINT8 Repaint)
{
	...
	if(pPara->StampMode == PRV_STAMP_DATETIME){
			 ...
			 Repaint) 
		{
			...
			xpos = ((LCD_WIDTH - totalW) >> 1) + strWidthGet(strDate);
			 ...
			 Repaint) 
		{
			...
			xpos = ((LCD_WIDTH - totalW) >> 1) + strWidthGet(strDate) + strWidthGet(strHourMinute);
...
void AF_AfCalStatusShow(UINT8 status, UINT8 flag) {
	...
	if (flag) {
		sp1kOsdSelWinSet(xs, ys, width, height, 2, (status ? OSD_ATTR_COLOR_GREEN : OSD_ATTR_COLOR_RED));
	} else {
