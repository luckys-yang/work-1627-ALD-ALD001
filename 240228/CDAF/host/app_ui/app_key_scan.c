...
extern UINT8 disablebtnflag;    // 禁用按键标志位
...
static keyMsg_t code keyMsg[SP1K_BTN_NUM] = {
    ...
    /*ADC A button msg*/
    //{SP1K_MSG_KEY_PRESS_ZOOM_IN,		SP1K_MSG_KEY_REPEATE_ZOOM_IN, 		SP1K_MSG_KEY_RELEASE_ZOOM_IN},
    //{SP1K_MSG_KEY_PRESS_SNAP, 			SP1K_MSG_KEY_REPEATE_SNAP, 			SP1K_MSG_KEY_RELEASE_SNAP},
    //{SP1K_MSG_KEY_PRESS_ZOOM_OUT,		SP1K_MSG_KEY_REPEATE_ZOOM_OUT, 		SP1K_MSG_KEY_RELEASE_ZOOM_OUT},
    {SP1K_MSG_KEY_PRESS_POWER, SP1K_MSG_KEY_REPEATE_POWER, SP1K_MSG_KEY_RELEASE_POWER},

    /*ADC B button msg*/
    {SP1K_MSG_KEY_PRESS_MODE, SP1K_MSG_NULL, SP1K_MSG_KEY_RELEASE_MODE},             // MODE
    {SP1K_MSG_KEY_PRESS_PB, SP1K_MSG_KEY_REPEATE_PB, SP1K_MSG_KEY_RELEASE_PB},       // DEL键
    {SP1K_MSG_KEY_PRESS_DOWN, SP1K_MSG_KEY_REPEATE_DOWN, SP1K_MSG_KEY_RELEASE_DOWN}, // DOWN
    {SP1K_MSG_KEY_PRESS_MENU, SP1K_MSG_KEY_REPEATE_MENU, SP1K_MSG_KEY_RELEASE_MENU}, // MENU
    {SP1K_MSG_KEY_PRESS_AF, SP1K_MSG_KEY_REPEATE_AF, SP1K_MSG_KEY_RELEASE_AF},       // BACK
    ...
    /*GPIO button msg*/
    {SP1K_MSG_KEY_PRESS_ZOOM_OUT, SP1K_MSG_KEY_REPEATE_ZOOM_OUT, SP1K_MSG_KEY_RELEASE_ZOOM_OUT}, //w(放大)
    {SP1K_MSG_KEY_PRESS_SNAP, SP1K_MSG_KEY_REPEATE_SNAP, SP1K_MSG_KEY_RELEASE_SNAP},    // 【拍照第二段】
    {SP1K_MSG_KEY_PRESS_ZOOM_IN, SP1K_MSG_KEY_REPEATE_ZOOM_IN, SP1K_MSG_KEY_RELEASE_ZOOM_IN},   //t(缩小)
    {SP1K_MSG_KEY_PRESS_AF_KEY, NULL, NULL},    // 【AF_Key拍照第一段】
...
UINT8 AF_MdAction1SecCnt = 0;
UINT8 AF_MdAction1SecCntStartFlag = 0;
UINT8 AF_MdAction1SecDoneFlag = 0;
#endif
...
void keyScan(void)
{
    ...
    static UINT16 af_timer_cnt = AF_AUTO_FOCUS_TIMER;  // 用于自动对焦计时器 
    static UINT16 af_back_far_cnt = AF_BACK_FAR_TIMER;
    static UINT16 video_af_timer_cnt = VIDEO_AF_TIMER;
	...
	if ((++uiOSDTime >= 100) && (puiPara->StampMode != PRV_STAMP_OFF) && ((appCurrStateGet() == APP_STATE_STILL_VIEW) || (appCurrStateGet() == APP_STATE_VIDEO_VIEW)))  {
		uiOSDTime = 0;			
    ...
    if (VideoPara.video_af_timer_start)
    {
        if (--video_af_timer_cnt == 0)
        {
            VideoPara.video_af_timer_start = Flag_false;
            video_af_timer_cnt = VIDEO_AF_TIMER;
            VideoPara.video_af_timer_out = Flag_true;
        }
    }
    else
    {
    ...
    if ((Flag_true == StillPara.af_far_flag) && (Flag_false == StillPara.self_snap_back_far_flag))
    {
        if (--af_back_far_cnt == 0)
        {
            StillPara.self_snap_back_far_flag = Flag_false;
            StillPara.af_far_flag = Flag_false;
            af_back_far_cnt = AF_BACK_FAR_TIMER;
    ...
    // 自动对焦计时器逻辑
    if (StillPara.still_af_timer_start)
    {
        if (--af_timer_cnt == 0)
        {
            StillPara.still_af_timer_start = Flag_false;
            af_timer_cnt = AF_AUTO_FOCUS_TIMER;
            StillPara.still_af_timer_out = Flag_true;

            osMsgPost(SP1K_MSG_KEY_PRESS_AF_KEY);  // 发送按键自动对焦消息
        }
    }
    else
    {
	...
	if(puiPara->SnapAf == 1 ){
		if (AF_MdAfEnGet()){
			if (AF_MdAction1SecCntStartFlag) {
				if (AF_MdAction1SecCnt) {
					if (--AF_MdAction1SecCnt == 0) {
						AF_MdAction1SecCntStartFlag = 0;
						AF_MdAction1SecDoneFlag = 1;
						appAFCtrl(1);
...
/*-------------------- USER CODE: Custom Begin --------------------*/
    ...
    gpioChn = 3; // AF key gpio 2

    if (*pGpio->keyAttr[gpioChn].pKeyEn)
    {
        sp1kPullEnSet(4, 0x08, 0x08);
        sp1kPullSelSet(2, 0x80, 0x80);
        HAL_GpioByteDirSet_Intr(HAL_GPIO_BYTE_GEN0, 0x04, 0x00);
...
static void keyScanAdcParsing(keyUnitAdc_t* pAdc)
{
	...
	if (keyScanAdcIdleChk(pAdc->keyCtrl.keyValPrev, pAdc) == TRUE) {
		...
		if (pAdc->keyAttr.pKeyEn[pAdc->keyActiveIdx] & KEY_STATUS_PRESSED) {
			//if ((adcVal >= (947 - 30) && adcVal <= (947 + 30))||(adcVal >= (1004 - 30) && adcVal <= (1004 + 30))){
			//	pAdc->keyCtrl.keyValRpt = KEY_SCAN_ZOOM_ADC_VAL_RPT_START;
			//}else{
		...
		//if((pAdc->keyActiveIdx == 2)&&(pAdc->keyAttr.keyType == 1)){
...
static void keyScanDgpioValParsing(keyUnitAdc_t* pAdc)
{
	...
	else
	{
		if ( !(keyValue & 0x02) ) {   // dgpio1 left
			...
			curKeyVal = 2;
		} else if ( !(keyValue & 0x08) ) {//dgpio2 right
			curKeyVal = 3;
		} else {
			if( prevKeyVal < 3 ) {
				if (pAdc->keyAttr.pKeyEn[pAdc->keyActiveIdx]& KEY_STATUS_RELEASED ){
