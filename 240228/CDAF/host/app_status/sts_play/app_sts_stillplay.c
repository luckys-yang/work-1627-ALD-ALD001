...
//	powerBatLv_t batlv;
    ...
    case SP1K_MSG_KEY_PRESS_MENU: // 【MENU键按下消息】
    {
        // 清除当前选择索引
        cur_idx = 0;
        // 清除删除标志
        ...
        switch (StillPara.play_return_view_Status)
        {
            case PAGE_STILL:
            {
                osMsgPost(SP1K_MSG_STATE_TO_STILL_VIEW);
        ...
        StillPara.play_return_view_Status = PAGE_NULL;
        break;
    }
    ...
    case SP1K_MSG_SYS_QUICK_DELETE: // 【BACK键消息】
    {
    ...
    case SP1K_MSG_KEY_PRESS_AF:
    {
        if (pb_delete_flag)
        {
            pb_delete_flag = Flag_false;
        }
        else
        {
            pb_delete_flag = Flag_true;
        }
        osMsgPost(SP1K_MSG_SYS_QUICK_DELETE); // 发送删除框消息
    ...
    case SP1K_MSG_KEY_PRESS_MODE:
    {
        ...
        if (!app_delete_key_hot_delete_sta_get())
        {
            ...
            case APP_STATE_VIDEO_VIEW:
                msg = SP1K_MSG_STATE_TO_STILL_VIEW; // SP1K_MSG_STATE_TO_VIDEO_VIEW;
                break;
