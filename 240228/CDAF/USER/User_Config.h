...
#define AF_BACK_FAR_TIMER 500   // 
#define VIDEO_AF_TIMER 100
/*===============================  控制宏定义 ============================*/
...
{
    PAGE_NULL = 0,  // 无需返回
    PAGE_STILL = 1, // 返回Still
    PAGE_VIDEO = 2  // 返回Video
} SysPage_et;
...
typedef struct
{
    UINT8 play_return_view_Status;    // 回放返回上一个页面状态
    UINT8 still_af_timer_start;             // 拍照定时器开启标志
    UINT8 still_af_timer_out;               // 拍照定时器超时标志
    UINT8 af_far_flag;                // 远景标志
    UINT8 self_snap_back_far_flag;        // 自拍后远景标志
} StillPara_st;
...
typedef struct
{
    UINT8 video_af_timer_start; // 录像定时器开启标志
    UINT8 video_af_timer_out;   // 录像定时器超时标志
} VideoPara_st;

extern StillPara_st StillPara;
extern VideoPara_st VideoPara;
